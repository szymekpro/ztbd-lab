...
Wykresy:
  ...
  3. INSERT – porównanie 3 skal (grouped bar)
    ...
    3c. UPDATE – avg_seconds per scenario (grouped bar by scale, with_indexes)
...
# ---------------------------------------------------------------------------
# PLOT X: UPDATE – avg time per scenario grouped by scale (like DELETE)
# ---------------------------------------------------------------------------

def plot_update_time_per_scenario(df: DataFrame, out_dir: Path, dpi: int) -> None:
    if "scale" not in df.columns:
        print("  [POMINIĘTO] UPDATE avg time per scenario: brak kolumny 'scale' w CSV")
        return
    ...
    ax.set_xticklabels(scenarios, rotation=20, ha="right", fontsize=9)
    ax.set_ylabel("Średni czas [s]")
    ax.set_title("UPDATE – Średni czas wykonania per scenariusz (z indeksami)")
    ax.legend(title="Skala danych")
    ax.yaxis.set_minor_locator(mticker.AutoMinorLocator())
    ax.grid(axis="y", alpha=0.4)
    fig.tight_layout()
    savefig(fig, out_dir, "update_avg_time_per_scenario.png", dpi)
...
def main() -> int:
    ...
    if update_df_multi is not None:
        print("[5/8] UPDATE – czas per scenariusz (skale)")
        plot_update_time_per_scenario(update_df_multi, out_dir, args.dpi)
